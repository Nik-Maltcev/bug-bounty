...
const groupDates = (items: { created_at?: string; timestamp?: string }[]) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <Link to="/programs" className="group relative bg-slate-900/60 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 transition-all hover:bg-slate-800/80 hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-900/20 overflow-hidden">
        ...
        <Link to="/vulnerabilities" className="group relative bg-slate-900/60 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 transition-all hover:bg-slate-800/80 hover:border-amber-500/50 hover:shadow-lg hover:shadow-amber-900/20 overflow-hidden">
          <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/vulnerabilities" className="group relative bg-slate-900/60 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 transition-all hover:bg-slate-800/80 hover:border-red-500/50 hover:shadow-lg hover:shadow-red-900/20 overflow-hidden">
          <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/audit" className="group relative bg-slate-900/60 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 transition-all hover:bg-slate-800/80 hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-900/20 overflow-hidden">
          <div className="flex items-center justify-between mb-4">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <section className="bg-slate-900/60 backdrop-blur-sm border border-slate-800 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/80">
              ...
              <Link to="/vulnerabilities" className="text-sm text-blue-400 hover:text-blue-300 transition-colors flex items-center">
          ...
          <section className="bg-slate-900/60 backdrop-blur-sm border border-slate-800 rounded-2xl shadow-sm overflow-hidden flex flex-col h-full">
            <div className="p-6 border-b border-slate-800 bg-slate-900/80">
              <h2 className="text-lg font-semibold text-slate-100 flex items-center justify-between">
                Недавние программы
                <Link to="/programs" className="p-1 rounded hover:bg-slate-800 text-slate-400 hover:text-slate-200 transition-colors">
            ...
            <div className="p-4 flex-1">
              {activePrograms.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Link to="/programs" className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition-colors">
                    Добавить программу
                  </Link>
                </div>
              ) : (
                <ul className="space-y-3">
                  {activePrograms.slice(0, 5).map((p) => (
                    <li key={p.id}>
                      <Link to={`/programs/${p.id}`} className="block p-3 rounded-xl border border-slate-800 hover:border-slate-700 hover:bg-slate-800/50 transition-all group">
